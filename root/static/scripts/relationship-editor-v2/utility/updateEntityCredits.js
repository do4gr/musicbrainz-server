...
import {cloneRelationshipState} from './cloneState';
import getRelationshipEditStatus from './getRelationshipEditStatus';
import isRelationshipBackward from './isRelationshipBackward';
import {ADD_RELATIONSHIP, REMOVE_RELATIONSHIP} from './updateRelationships';
import type {RelationshipUpdateT} from './updateRelationships';
import {
...
export default function* updateEntityCredits(
  ...
  const findLinkTypeGroupsForSameEntityType = () => {
    ...
    if (targetTypeGroupNode) {
      const [/* targetType */, linkTypeGroups] = targetTypeGroupNode.value;
  ...
  switch (creditsToChange) {
    ...
    case 'same-relationship-type': {
      ...
      if (linkTypeGroupNode) {
        yield *getCreditUpdatesForLinkTypeGroup(linkTypeGroupNode.value);
      }
      break;
    }
  }
